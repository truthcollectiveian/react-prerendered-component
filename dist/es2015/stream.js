...
export var sequenceParser = function (html, markers) {
...
export var splitFirst = function (str, needle) {
    var position = str.indexOf(needle);
    return position >= 0
        ? [str.substr(0, position), str.substr(position + 1)]
        : [str];
};
export var nextIndexOf = function (str, needles, index) {
...
export var toTemplateVariables = function (variables) {
    ...
    variables.forEach(function (str, index) {
        if (index > 0) {
            var _a = splitFirst(str, '='), key = _a[0], value = _a[1];
...
export var restore = function (variables, chunk, cache) {
    return process(chunk, lineVariables(variables), cache);
};
export var process = function (chunk, line, cache) {
    ...
    function parseTag(tag) {
        ...
        if (stind <= 0) {
            ...
            templatePush = tag;
            push = tag[0] + process(tag.substr(1, tag.length - 2), lineVariables(line.cache[line.stack[line.stack.length - 1]].variables), cache) + tag[tag.length - 1];
        }
    }
...
export var cacheRenderedToString = function (str, cache) { return (str.indexOf('x-cached') > 0
    ? process(str, createLine(), cache)
    : str); };
export var createCacheStream = function (cache) {
    var line = createLine();
